...
**`--tools` 只管内置工具,别的一概不管。** 实测(Claude Code 2.1.277):一个班次带
`--tools "Read,Grep,Write,Bash"` 启动,内置工具确实只剩这四个 —— 但 **84 个 MCP 工具照样在**,
其中包括 Google Drive 的 `share_file` 和 `trash_file`。**这些调用能不能真的走通,取决于用户级的
放行清单,这一点没有实测** —— 要实测就等于真去碰一次外部服务。
...
- **`--strict-mcp-config`** —— 旁边不带 `--mcp-config` 时,这个班次就一个 MCP 服务都没有。
- **`--setting-sources project,local`** —— 把用户级 settings 排除掉。不加,班次会继承整台机器的
  配置:实测一个冒烟班次因此多出 84 个工具、约 **85K token 的启动上下文,成本从 $0.056 涨到 $1.54**。

**权限规则的写法很讲究,写错了等于没写(实测)。** 路径放行要写成 `Edit(<路径>/**)`,它同时管住
`Write`;`Write(<路径>)` 这种写法**不生效**。另外,一旦排除了用户级 settings,读协调目录就必须
显式放行 `Read(<协调目录>/**)`,否则班次连自己的工单都读不到。

**沙箱会挡住 Keychain。** 沙箱里的班次根本起不了子会话:`claude -p` 约 20ms 就返回
`Not logged in`,把网络放行也没用。所以**派班次的那一方自己不能开沙箱** —— 沙箱是留给工位的。
...
**有一个例外:审查工位建议独立克隆 + 沙箱。** 在 worktree 上,「零写权限」只是纪律,不是物理约束 —— `.git` 是共用的。实测:在 worktree 上、即使开了 Claude Code 沙箱,审查方仍然能跑 `git -C <共享仓库> branch` 把分支建出来(**确实建成了,随后已删**)。换成**独立克隆 + 沙箱**之后,同一条命令被系统拒绝 —— `Operation not permitted`,协调目录也写不进去。**规矩和边界的差别就在这里。**
